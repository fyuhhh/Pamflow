...
const BuatTugas = ({ taskType = 'checklist' }) => {
  ...
  useEffect(() => {
    ...
    if (state?.fromAudit) {
      ...
      const auditDetails = state.brokenItems.map((item, idx) => ({
        id: Date.now() + idx,
        nama_detail: item.name,
        bentuk_laporan: 'Checkbox',
        deskripsi: item.notes || 'Perbaikan temuan audit',
        wajib_diisi: true,
      ...
      setFormData(prev => ({
        ...prev,
        nama_tugas: `Perbaikan ${state.templateName || 'Temuan'} - ${state.shift || ''}`,
  ...
  const [formData, setFormData] = useState(() => {
    ...
    if (auditData?.fromAudit) {
      const auditDetails = auditData.brokenItems.map((item, idx) => ({
        id: Date.now() + idx,
        nama_detail: item.name,
        bentuk_laporan: 'Checkbox',
        deskripsi: item.notes || 'Perbaikan temuan audit',
      ...
      return {
        ...base,
        nama_tugas: `Perbaikan ${auditData.templateName || 'Temuan'} - ${auditData.shift || ''}`,
        departemen: auditData.targetDept || base.departemen,
        deskripsi: `WO dibuat otomatis dari audit #SES-${auditData.session_id}.`,
        urgensi: 'Tinggi',
        aturan_waktu: 'Waktu Fleksibel',
        tanggal_mulai: new Date().toISOString().split('T')[0],
        tanggal_selesai: new Date(Date.now() + 86400000).toISOString().split('T')[0],
      ...
      const fetchDeptTask = async () => {
        try {
          ...
          if (response.ok) {
            ...
            const mappedDetails = sourceItems.length > 0 ? sourceItems.map((item, idx) => ({
              id: Date.now() + idx,
              nama_detail: item.name,
              bentuk_laporan: 'Checkbox',
            ...
            })) : (task.details || prev.details);

            // If it has wo_items, treat it like an audit for UI purposes
            if (sourceItems.length > 0) {
               setAuditData({
                 session_id: task.checklist_session_id || 'WO',
  ...
  useEffect(() => {
    if (sourceDeptTask?.template_id && templates.length > 0 && !templateAutoAppliedRef.current) {
  ...
  const handleSubmit = async (statusOverride = null) => {
    ...
    try {
      ...
      if (response.ok) {
        success('Berhasil', isEditMode ? 'Tugas berhasil diperbarui!' : 'Tugas berhasil dibuat!');
        ...
        {step === 1 && (
          <div className="p-8 space-y-10">
            ...
            <div>
              ...
              {auditData && formData.details.length > 0 && (
                ...
                >
                  ...
                  <div className="flex items-center justify-between px-6 py-4 border-b border-[#F1F1F4]">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-sm font-bold text-[#3F4254]">Detail Tugas ke {index + 1}</span>
                    </div>
                    <div className="flex items-center gap-4">
