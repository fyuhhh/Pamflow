...
async function migrate() {
  ...
  try {
    ...
    for (const asset of assets) {
      const processed = await processBase64InObject(asset.lampiran, 'assets');
    ...
    for (const log of auditLogs) {
      const processed = await processBase64InObject(log.photos, 'assets');
    ...
    for (const task of tasks) {
      const processed = await processBase64InObject(task.submission_data, 'tasks');
    ...
    for (const session of sessions) {
      const processed = await processBase64InObject(session.item_results, 'checklists');
    ...
    for (const dt of deptTasks) {
      ...
      if (dt.lampiran) {
        const proc = await processBase64InObject(dt.lampiran, 'dept-tasks');
      ...
      if (dt.wo_items) {
        const proc = await processBase64InObject(dt.wo_items, 'dept-tasks');
      ...
      if (dt.partial_submissions) {
        const proc = await processBase64InObject(dt.partial_submissions, 'dept-tasks');
